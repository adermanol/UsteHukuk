...
3. SQL Editor'de **sadece** şu migration'ları, sırayla çalıştırın:
   ...
   **`00000000000000_schema.sql`'i çalıştırmayın.** Bu, projede fiilen kullanılmayan eski bir CMS tablo taslağıdır (`general_settings`, `hero_section`, vb.) — gerçek site içeriği ve LLM sağlayıcı ayarları `app_settings` tablosunda (`key='cms_data'` / `key='llm_settings'`) tutulur, bu eski taslak tablolarda değil. Çalıştırmak zarar vermez ama gereksiz karışıklık yaratır.
   ...
   >
   > **ÖNEMLİ — kalıcı bir gotcha:** Bu taşımadan sonra canlı site içeriği artık `data/cms-data.json` dosyasından DEĞİL, doğrudan `app_settings` tablosundaki `cms_data` satırından okunur (`src/lib/cms.ts`). Yani `data/cms-data.json`'da (örn. yeni bir dil çevirisi eklendiğinde) yapılan bir değişiklik, tabloya AYRICA yazılmadan canlıya asla yansımaz — dosyayı güncellemek tek başına yeterli değildir. Böyle bir değişiklikten sonra `node scripts/seed-app-settings.js`'i tekrar çalıştırın (veya yalnızca `cms_data` anahtarını güncelleyen, `llm_settings`'e dokunmayan dar kapsamlı bir upsert kullanın — `llm_settings` genelde `/dashboard/settings` üzerinden ayrıca değiştirilmiş olabilir, seed script'i onu da ezer).
